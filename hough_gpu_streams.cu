...
__global__ void fill_accum_from_coords(int* accum,int* x_coords,int* y_coords,int *num_points,float* cos_table,float* sin_table,int numangle,int numrho, int x_offset, int y_offset){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
std::pair<int,line*> hough_parallel_segmented(unsigned char* d_img, int N, int threshold,
    ...
    cudaMalloc(&d_sinvalues, numangle * sizeof(float));
    cudaMalloc(&d_cosvalues, numangle * sizeof(float));
    ...
    fill_trig_tables<<<(numangle+255)/256, 256, 0, streams[0]>>>(d_sinvalues, d_cosvalues, min_theta, theta_step, numangle, irho);
    ...
    fill_accum_from_coords<<<(threads+255)/256, 256, 0, streams[0]>>>(acc1, x1, y1, cnt1, d_cosvalues, d_sinvalues, numangle, numrho, 0, 0);
    fill_accum_from_coords<<<(threads+255)/256, 256, 0, streams[1]>>>(acc2, x2, y2, cnt2, d_cosvalues, d_sinvalues, numangle, numrho, halfN, 0);
    fill_accum_from_coords<<<(threads+255)/256, 256, 0, streams[2]>>>(acc3, x3, y3, cnt3, d_cosvalues, d_sinvalues, numangle, numrho, 0, halfN);
    fill_accum_from_coords<<<(threads+255)/256, 256, 0, streams[3]>>>(acc4, x4, y4, cnt4, d_cosvalues, d_sinvalues, numangle, numrho, halfN, halfN);
    ...
    cudaFree(d_sinvalues); cudaFree(d_cosvalues);
